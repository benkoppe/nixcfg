...
function buildPrompt(mode: LoopMode, condition?: string): string {
  switch (mode) {
    ...
    case "custom":
      return (
        `Continue until the following condition is satisfied: ${conditionText(
          mode,
          condition,
...
export default function (pi: ExtensionAPI): void {
  ...
  function refreshSummary(
    ...
    void summarize(ctx, mode, condition).then((s) => {
      if (!state.active || state.mode !== mode || state.condition !== condition)
        return;
      ...
      ? [
          {
            ...
            description:
              state.summary ?? conditionText(state.mode!, state.condition),
          },
          ...PRESETS,
        ]
      : PRESETS;
  ...
  pi.registerTool({
    ...
    async execute(_id, _p, _sig, _upd, ctx) {
      ...
      const text = wasActive ? "Loop ended." : "No active loop is running.";

      if (wasActive) clear(ctx);
  ...
  pi.on("agent_end", async (ev, ctx) => {
    ...
    const aborted =
      ev.messages.findLast((m) => m.role === "assistant")?.stopReason ===
      "aborted";
